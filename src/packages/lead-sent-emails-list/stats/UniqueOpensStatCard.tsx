...
export const UniqueOpensStatCard = () => {
  ...
  const count = useMemo(() => {
    ...
    const opened = filteredEmails.filter((e) => e.delivery_status === 'opened');
  ...
  const handleClick = useCallback(() => {
    if (isActive) {
      ...
      dispatch(LeadSentEmailsBuilderActions.setStatCardFilter('unique_opens'));
    }
  }, [dispatch, isActive]);
  ...
  return (
    ...
    >
      <span className={styles.metricLabel}>Unique Opens</span>
