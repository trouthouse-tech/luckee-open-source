...
export const BouncedStatCard = () => {
  ...
  const isActive = statCardFilter === 'bounced';
  ...
  const count = useMemo(() => {
    ...
    return filteredEmails.filter((e) => e.delivery_status === 'bounced').length;
  ...
  const handleClick = useCallback(() => {
    if (isActive) {
      ...
      dispatch(LeadSentEmailsBuilderActions.setStatCardFilter('bounced'));
    }
  }, [dispatch, isActive]);
  ...
  return (
    ...
    >
      <span className={styles.metricLabel}>Bounced</span>
